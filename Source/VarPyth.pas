  ...
  begin
    ...
    begin
      ...
      case ArgType of
        ...
        varSingle:    PVarParm^.VSingle := PSingle(Params)^;
        varDouble:    PVarParm^.VDouble := PDouble(Params)^;
        varCurrency:  PVarParm^.VCurrency := PCurrency(Params)^;
        varDate:      PVarParm^.VDate := PDateTime(Params)^;
        ...
        begin
          ...
          {$ELSE}
          PVariant(PVarParm)^ := PVariant(Params^)^; //it arrives here as a pointer for 32bits also
          {$ENDIF}
        end;
